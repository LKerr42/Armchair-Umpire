...
async function importLeague(leagueJson) {
    ...
    const query = `
        INSERT INTO leagues (l_api_id, l_name, l_name_alt, l_curr_season,
                                    l_year_formed, l_gender, l_country,
                                    l_website, l_twitter, l_youtube, l_instagram, l_facebook,
                                    t_description, l_naming)
...
async function importFullLeague(leagueApiId) {
    const teams = await fetchLeagueTeams(leagueApiId);
    console.log(teams);

    console.log(teams.row[0]);

    const leagueData = await pool.query(`SELECT l_id FROM leagues WHERE l_api_id = $1;`, [leagueApiId]);
    leagueID = leagueData.rows[0].l_id;

    console.log(leagueID);

    // for (let i = 0; i < )

    //importTeam(teams.row[0], leagueID);
}
